...
void updateDayNight(int fd,bool giorno);
...
void loadScenariPresenzeTable(bool onlyupdate);
	...
	if(sendLen>1)
	{
		...
		* rappresentante i valori di invio
		...
		for(i=0;i<out_ch_d;i++)
		{
			...
			{
				//verifico che non ci sia un poweron time
				if((digitalOutTable[deviceToid[2][deviceId][i]].potime>0)
					&&(digitalOutTable[deviceToid[2][deviceId][i]].start_time>0))
				{
					if((time(NULL)-digitalOutTable[deviceToid[2][deviceId][i]].start_time)
							>=digitalOutTable[deviceToid[2][deviceId][i]].potime)
					{
						digitalOutTable[deviceToid[2][deviceId][i]].value=digitalOutTable[deviceToid[2][deviceId][i]].def;
						digitalOutTable[deviceToid[2][deviceId][i]].start_time=0;
					}
				}
				//fine verifica;
	...
	if(strstr(buf,"out ")==buf)
	{
		if(c=strchr(&buf[4],' '))
		{
			*c='\0';
			...
			printf("id_digital_out\tdescription\tdevice_num\tch_num\tvalue\tpotime\tstart\n");
			printf("%d\t%s\t%d\t%d\t%d\t%d\t%d\n",digitalOutTable[i].id_digital_out,
						...
						digitalOutTable[i].ch_num,digitalOutTable[i].potime,
						(int)digitalOutTable[i].ch_num,digitalOutTable[i].start_time);
		}
		return;
	}
...
void updateDayNight(int fd,bool boolGiorno)
{
	int state;
	MYSQL_RES *result;
	MYSQL_ROW row;
	...
	giorno=(boolGiorno?1:0);
	sprintf(query,"SELECT scenari_giorno_notte.id_digital_out,scenari_giorno_notte.ritardo,digital_out.`default` "
						"FROM scenari_giorno_notte INNER JOIN digital_out "
						"ON scenari_giorno_notte.id_digital_out=digital_out.id_digital_out "
						"WHERE scenari_giorno_notte.giorno=%d AND scenari_giorno_notte.attivo=1",giorno);
	...
	while (( row = mysql_fetch_row(result)) != NULL )
	{
		id_digital_out=atoi(row[0]);
		ritardo=atoi(row[1]);
		def=atoi(row[2]);
		j=id_digitalOutToId(id_digital_out);//source index

		digitalOutTable[j].start_time=time(NULL);
		digitalOutTable[j].def=1-giorno+(2*giorno-1)*def;
		digitalOutTable[j].potime=ritardo;
	}
	mysql_free_result(result);	
	if(!ko)
		strcpy(ok,"ok");
		...
		if(i!=-1)	//canale di destinazione >0
		{
			...
			if(j!=-1) //sorgente era associato 
			{
				if(systems[systemId].sockfd!=-1)
				{
					...
					digitalOutTable[i].potime=digitalOutTable[j].potime;
				}
				else
					initializeDigitalOutLine(i);
			...
			{
				digitalOutTable[i].start_time=time(NULL);
				digitalOutTable[i].value=1-digitalOutTable[i].def;
			}
...
void setOutputDefault(int systemId)
{
	...
	for(i=0;i<systems[systemId].out_ch_d;i++)
	{
		...
		if(digitalOutTable[idRow].id_digital_out!=-1)
		{
			...
			{
				digitalOutTable[idRow].value=1-digitalOutTable[idRow].def;
				digitalOutTable[idRow].start_time=0;
			}
			else
				digitalOutTable[idRow].value=digitalOutTable[idRow].def;
		}
