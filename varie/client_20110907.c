...
void initializeIrrigazioneTable();
...
int loadIrrigazioneTable(int id_irrigazione,int circuito);
...
// END READ EFFEMERIDI
	...
	pid=(int *)malloc((NUMSYSTEMS+NUMMULTIMETERS) * sizeof(int));
	deviceToid=(int ***)malloc(4*sizeof(int **));
...
//FINE MYSQL STUFF
	...
	for(k=0;k<NUMSYSTEMS+NUMMULTIMETERS;k++)
	{
		pid[k]=-1;
 ...
 * -----------------------------------------*/
{
	...
	for(i=0;i<IRRIGAZIONECIRCUITS;i++)
	{
		irrigazioneTable[i].id_irrigazione=-1;
		irrigazioneTable[i].circuito=-1;
		irrigazioneTable[i].ora_start=-1;
		irrigazioneTable[i].durata=-1;
		irrigazioneTable[i].validita=-1;
...
void resetIrrigazioneValues(int i)
{
	irrigazioneTable[i].tempo_off_cur=-1;
}
...
void sendResetSignal(int systemId)
{
	...
	if(bytes_sent < 0)
	{
		perror("Error sending first reset packet: %s\n");
	...
	if(bytes_sent < 0)
	{
		perror("Error sending  second reset packet: %s\n");
		close(sock);//close the socket
		return;
	}
	totbytes+=bytes_sent;
...
int circuito_irrigazioneToId(int id_irrigazione,int circuito_irrigazione)
{
	...
	for(i=0;i<IRRIGAZIONECIRCUITS;i++)
		if((irrigazioneTable[i].id_irrigazione==id_irrigazione)&&
				(irrigazioneTable[i].circuito==circuito_irrigazione))
			break;
	return((i<IRRIGAZIONECIRCUITS)?i:-1);
}
...
int loadIrrigazioneTable(int id_irrigazione,int circuito)
{
	// circuito=0 => crea tabella
	// circuito>0 => aggiorna circuito
	// circuito<0 => aggiorna tabella
	...
	sprintf(query,"SELECT irrigazione.id as id_irrigazione,"
						...
						"WHERE irrigazione.id=%d ",id_irrigazione);
	...
	if(circuito>0)
	{
		...
		sprintf(query,"%s AND irrigazione_circuiti.circuito=%d",query,circuito);
	}
	sprintf(query,"%s ORDER BY irrigazione.id,irrigazione_circuiti.circuito",query);
	...
	if(circuito==0)
	{
		IRRIGAZIONECIRCUITS=mysql_num_rows(result);
		irrigazioneTable=(struct irrigazioneLine *)get_shared_memory_segment(IRRIGAZIONECIRCUITS * sizeof(struct irrigazioneLine), &SHMIRRIGAZIONEID, "/dev/zero");
		if(!irrigazioneTable)
	...
	if(circuito<=0)
		initializeIrrigazioneTable();

	while( ( row = mysql_fetch_row(result)) != NULL )
	{
		irrigazioneTable[idRow].id_irrigazione=atoi(row[0]);
		irrigazioneTable[idRow].circuito=atoi(row[1]);
		irrigazioneTable[idRow].ora_start=atoi(row[2]);
		irrigazioneTable[idRow].durata=atoi(row[3]);
		irrigazioneTable[idRow].validita=atoi(row[4]);
		irrigazioneTable[idRow].ripetitivita=atoi(row[5]);
		irrigazioneTable[idRow].tempo_off=atoi(row[6]);
		irrigazioneTable[idRow].tempo_off_cur=0;
		idRow++;
	}
	mysql_free_result(result);
...
int loadDigitalOutTable(bool onlyupdate)
{
	...
	state = mysql_query(connection, "SELECT digital_out.id_digital_out,digital_out.description,"
							...
							"digital_out.`default`,digital_out.po_delay,digital_out.on_value,on_time,pon_time,poff_time "
...
void set_irrigazione(int fd,char *c)
{
	...
	if(circuito>0)
	{
		...
		if(id!=-1)
		{
			...
			state = mysql_query(connection, query);
			if(state==0)
				loadIrrigazioneTable(1,circuito);
			sprintf(s2,"%d %d %d\n",irrigazioneTable[id].circuito,
				irrigazioneTable[id].durata,
				irrigazioneTable[id].validita);
		}
	}
	else
	{
