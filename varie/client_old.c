...
int id_digitialToId(int id_digital);
int id_digitialOutToId(int id);
...
//READ SYSTEM DEFAULTS
	state = mysql_query(connection, "SELECT record_data_time,localita,nome,reboot_time FROM system_ini");
...
//FINE MYSQL STUFF
	...
	do
	{
		...
		if(time(NULL)-db_counter >= RECORDDATATIME)
		{
			storeTables();
			db_counter=time(NULL);
		}
 ...
 *		digitalTable[id_digitialToId(1)].value;
 * 		setOutput(22,0);
 * 	
 * 		formatMessage(writeBuffer,0,0,0,0,"SYSTEM STOPPING");
 *		write(logFileFd,writeBuffer,strlen(writeBuffer));
...
void storeTables()
{
	...
	for(i=0;i<ANALOGCHANNELS;i++)
	{
		if((systems[systemNumToId(analogTable[i].device_num,NUMSYSTEMS)].enabled)
			...
			&&(analogTable[i].id_analog!=-1))
		{
			sprintf(query,"INSERT INTO history(`datetime`,`device_num`,`ch_num`,`value`,`raw`,`um`)" 
							" VALUES ('%s',%d,%d,%f,%d,'%s')",datetime,analogTable[i].device_num,
								analogTable[i].ch_num,analogTable[i].value_eng,analogTable[i].value,
								analogTable[i].unit);
...
void updateDigitalChannel(int id_digital)
{
	...
	if (( row = mysql_fetch_row(result)) != NULL )
	{
		j=id_digitialToId(id_digital);//source
...
void updateDigitalOutChannel(int id_digital_out)
{
	...
	if (( row = mysql_fetch_row(result)) != NULL )
	{
		j=id_digitialOutToId(id_digital_out);//source index
...
int id_digitialOutToId(int id_digital_out)
{
	int i;
