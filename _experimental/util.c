...
void scriviEffemeridiSuFile(int fd)
{
	...
	sprintf(buffer,"%02d:%02d:%02d\t0\t0\t0\t0\tcaricate effemeridi: alba %s - tramonto %s\n",
		hour,min,sec,dawn,sunset);
...
void readEffemeridi()
{
	...
	state = mysql_query(conn, "SELECT month,day,dawn,sunset FROM effemeridi ORDER BY month,day");
	if( state != 0 )
	{
		my_printf("%s\n",mysql_error(conn));
...
void my_printf(const char *format, ...) 
{
	...
	printf("%04d-%02d-%02d %02d:%02d:%02d - pid %d - %s", 
