...
/*			handleMessage(3,0,irrigazioneTable[id_irrigazione].id_irrigazione,
			...
			handleMessage(-1,0,0,0,irrigazioneTable[id_irrigazione].msg_avvio_pompa,0,ALARM_NO_STORE);
			activateOutputImmediate(id);
		}
	}
}
...
/*--------------------------------------------
 * 		attiva il circuito id_circuito su sistema id_irrigazione
 * 		disattiva gli altri
 * 		non controlla validità!!!
 * -----------------------------------------*/
{
	int i,j;
	int id_circuito_temp;

	for(i=0;i<irrigazioneTable[id_irrigazione].num_circuiti;i++)
	{
		id_circuito_temp=circuito_irrigazioneToId(irrigazioneTable[id_irrigazione].id_irrigazione,i+1);
		if(id_circuito_temp>=0)
		{
			j=id_digitalOutToId(irrigazioneCircuitiTable[id_circuito_temp].id_digital_out);
			if(j!=-1)
			{
				if(id_circuito_temp==id_circuito)
					activateOutputImmediate(j);
				else
					deactivateOutputImmediate(j);
			}
		}
	}
}
...
/*			handleMessage(3,0,irrigazioneTable[id_irrigazione].id_irrigazione,
			...
			handleMessage(-1,0,0,0,
				irrigazioneTable[id_irrigazione].msg_arresto_pompa,0,ALARM_NO_STORE);

			deactivateOutputImmediate(id);
		}
	}
}
...
//		handleMessage(3,0,irrigazioneTable[id_irrigazione].id_irrigazione,0,message,0,1);
		handleMessage(-1,0,0,0,message,0,ALARM_NO_STORE);
...
/*		handleMessage(3,id_circuito,irrigazioneTable[id_irrigazione].id_irrigazione,
	...
	irrigazioneTable[id_irrigazione].attivo_forzato=0;
	irrigazioneTable[id_irrigazione].current_time=0;
	...
	else //id_irrigazione > 0
	{
		...
		else
		{
			...
			if( ( row = mysql_fetch_row(result)) != NULL )
			{
				...
				irrigazioneTable[idRow].attivo_forzato=0;
			}
			mysql_free_result(result);
...
void get_irrigazione(int fd,char *c)
{
	...
	if( state != 0 )
	{
		sprintf(buffer,"%s %s",mysql_error(conn),TERMINATOR);
		if (send(fd, buffer,strlen(buffer), 0) == -1)
			my_perror("send");
		return;
	}
	...
	if(atoi(c)>0)
	{
		...
		if(id_irrigazione!=-1)
		{
			...
			sprintf(buffer,"%d\t%d\t%d\t%d\t%d\t%d\t%ld\t%d\t%d\t%d\n",
				...
				irrigazioneTable[id_irrigazione].attivo_forzato,
	...
	else
	{
		for(i=0;i<IRRIGAZIONESYSTEMS;i++)
		{
			sprintf(buffer,"%d %d %d %d %d %d %ld %d %d %d\n",
				...
				irrigazioneTable[i].attivo_forzato,
...
void doIrrigazione()
{
	...
	int i,j,id_circuito,circuito,id_irrigazione,num_circuiti,day,wday;
	struct msgform msg;
	...
	for(i=0;i<IRRIGAZIONESYSTEMS;i++)
	{
		id_irrigazione=irrigazioneTable[i].id_irrigazione;
		num_circuiti=irrigazioneTable[i].num_circuiti;
			...
			//può essere che è ancora attivo dal giorno prec
		{
			...
			if((irrigazioneTable[i].attivo_forzato==0)&&	// non forzato
...
/*						handleMessage(3,0,id_irrigazione,0,
							irrigazioneTable[i].msg_pioggia_noinizio,0,1);*/
						handleMessage(-1,0,0,0,
							irrigazioneTable[i].msg_pioggia_noinizio,0,ALARM_NO_STORE);
					}
				}
			}
			else
			{
				if((pioggia==1)
						&&(irrigazioneTable[i].attivo_forzato==0)
						&&(irrigazioneTable[i].current_circuito>0))
						// significa che è partito in automatico ma ora piove
			...
			{
				if(irrigazioneTable[i].current_circuito<=num_circuiti)
				{
					id_circuito=circuito_irrigazioneToId(id_irrigazione,irrigazioneTable[i].current_circuito);
					if(id_circuito!=-1)
					{
						if((irrigazioneTable[i].current_time-(seconds-irrigazioneTable[i].ora_started))>0) //circuito ancora attivo
						{
							...
							irrigazione_attiva_circuito(i,id_circuito);
						}
						else 	//circuito non più attivo
						{
							reset_irrigazione(i);
							irrigazione_prossimo(i,wday);
						}
					}
...
void irrigazione_prossimo(int i,int wday)
{
	...
	irrigazioneTable[i].ora_started=time(NULL);
	irrigazioneTable[i].current_circuito++;
	...
	{
		id_circuito=circuito_irrigazioneToId(irrigazioneTable[i].id_irrigazione,
								irrigazioneTable[i].current_circuito);
		if(irrigazioneCircuitiTable[id_circuito].validita & (1<<wday))
