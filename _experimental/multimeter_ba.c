			...
			if(headerok)
			{
				for(i=0;i<out_ch;i++)
				{
							...
							&&(readingTable[readingId].enabled==1))
					{
						...
						memcpy(readingTable[readingId].raw,&recvBuf[3+4*i],4);
						for(j=0;j<4;j++)
...
int loadReadingTable(bool onlyupdate)
{
	...
	while( ( row = mysql_fetch_row(result)) != NULL )
	{
		...
		if((i!=-1) && (atoi(row[3])<=(multimeters[i].out_ch_1+multimeters[i].out_ch_2)))
		{
			...
			readingTable[idRow].record_data_time=(atoi(row[23])!=-1?60*atoi(row[23]):RECORDDATATIME);
 ...
 * -----------------------------------------*/
{
	...
	FILE *pFile;
	char path[255];
	sprintf(path,"/home/zagico/socket/experimental/%s.txt",multimeters[multimeterId].address);

	if(strcmp(multimeters[multimeterId].address,"192.168.1.131")==0)
		my_printf("---------begin operating---------\n");
	sockfd=multimeters[multimeterId].sockfd;
	...
	else
	{
		...
		while((continua)&&(ch_num<out_ch))	// finchè non trovo 0x0f
		{
			...
			if(strcmp(multimeters[multimeterId].address,"192.168.1.131")==0)
			{
				sprintf(path,"/home/zagico/socket/_experimental/%s.txt",multimeters[multimeterId].address);
				pFile = fopen (path, "ab");
