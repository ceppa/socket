...
/*			handleMessage(3,0,irrigazioneTable[id_irrigazione].id_irrigazione,
			...
			handleMessage(-1,0,0,0,irrigazioneTable[id_irrigazione].msg_avvio_pompa,0,-1);
...
/*			handleMessage(3,0,irrigazioneTable[id_irrigazione].id_irrigazione,
			...
			handleMessage(-1,0,0,0,
				irrigazioneTable[id_irrigazione].msg_arresto_pompa,0,-1);
...
//		handleMessage(3,0,irrigazioneTable[id_irrigazione].id_irrigazione,0,message,0,1);
		handleMessage(-1,0,0,0,message,0,-1);
...
void get_irrigazione(int fd,char *c)
{
	...
	if( state != 0 )
	{
		sprintf(buffer,"%s %s",mysql_error(connection),TERMINATOR);
		if (send(fd, buffer,strlen(buffer), 0) == -1)
			my_perror("send");
		return;
	}
