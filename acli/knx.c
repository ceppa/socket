...
int operateKnx(int sockfd,int knxGatewayId, int timeout)
{
	...
	numbytes=recv(sockfd, &recvBuf[recvBufReceived], 
			recvLen-recvBufReceived, MSG_DONTWAIT);
...
//messaggi e allarmi
								...
								&&(strlen(knxTable[i].low_hi_msg)))))
						{
							...
							if(knxTable[i].is_alarm)
								messageType=(value==knxTable[i].alarm_transition?0:-2);
...
//fine messaggi e allarmi
						...
						switch(knxTable[i].data_type)
						{
							case 'F':
								...
								break;
							case 'C':
								knxTable[i].value_eng=(double)my_round(((double)(value))*100/255);
								break;
							case 'B':
								knxTable[i].value_eng=(value&1);
								break;
							default:
