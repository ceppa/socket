...
void doMultimeter(int l)
{
	...
	do
	{
		if(multimeters[l].enabled)
		{
			...
			if(multimeters[l].sockfd!=-1)
			{
				out=operateMultimeter(multimeters[l].sockfd,
					multmeterNumToId(multimeters[l].multimeter_num,NUMMULTIMETERS),2,i);
				if(out==-1)
				{
					handleMessage(MULTIMETER,0,multimeters[l].multimeter_num,
						0,"ERROR OPERATING, SLEEPING 5 SECONDS",0,0);
...
*/
	...
	handleMessage(MULTIMETER,0,multimeters[multimeterId].multimeter_num,0,"MULTIMETER CONNECTED",0,-2);
...
//gestione allarmi
						if(strlen(readingTable[readingId].msg))
						{
							if((reading>readingTable[readingId].alarm_value)
								&&(readingTable[readingId].value_valid==TRUE)
								&&(readingTable[readingId].value<readingTable[readingId].alarm_value))
								handleMessage(
										MULTIMETER,
										readingTable[readingId].id_reading,
										readingTable[readingId].multimeter_num,
										0,
										readingTable[readingId].msg,
										0,
										-1);
							if((reading<readingTable[readingId].alarm_value)
								&&(readingTable[readingId].value_valid==TRUE)
